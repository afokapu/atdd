...
| acc:spawn-agents:E031-SMOKE-001-real-respawn-leaves-no-orphan | real-entry-point harness (live AgentController + real worker process), live-on-demand `ATDD_LIVE_SMOKE=1` | prior worker PID gone, exactly one fresh process on the surface, no orphan/ghost | coach phase transition → respawn_worker (signal+stop+confirm) → agent_control is_alive → fresh spawn | #1079 (hermetic E031 unit tier carries the guarantee; live verification pending respawn_worker wiring into the coach transition path + `AgentController.is_alive` — skips cleanly until opted in) |
| acc:spawn-agents:E032-SMOKE-001-real-issue-advance-idempotent | real (live `gh` label API on a scratch issue via the real GREEN function's injected read/swap), live-on-demand `ATDD_LIVE_SMOKE=1` + `SMOKE_TEST_ISSUE` | first call lands exactly the target `atdd:<phase>` label, second call mutates nothing, neither errors | advance_phase_label_idempotent → gh read_phase / swap_label (label REST) | #1079 (hermetic E032 unit tier carries the guarantee; runnable now against a scratch issue; skips cleanly until opted in) |
| acc:spawn-agents:C002-SMOKE-001-real-cmux-paste-lands-in-live-surface | real-entry-point harness (live cmux surfaces), live-on-demand `ATDD_LIVE_SMOKE=1` | prompt observed in the live surface, nothing pasted into the forced stale/duplicate ref, exactly one live surface remains | guarded_paste → cmux surface registry (live_surfaces_for/is_live/create/reap/paste) | #1079 (hermetic C002 unit tier carries the guarantee; live verification pending the cmux surface-registry adapter — skips cleanly until opted in) |
| acc:spawn-agents:M003-SMOKE-001-real-hung-warm-resume-times-out | real-entry-point harness (live coach warm-resume), live-on-demand `ATDD_LIVE_SMOKE=1` | call returns within ~budget (not indefinitely), an escalation line names the issue + source→dest transition, no zombie spawn left | issue_runner warm-resume branch → run_warm_resume_with_timeout watchdog → escalation channel | #1079 (hermetic M003 unit tier carries the guarantee; live verification pending the watchdog wiring into the issue_runner warm-resume branch — skips cleanly until opted in) |
...
| real-entry-point / hermetic-backed (live pending wiring) | 4 | Real-infra harness, live-on-demand; live verification pending the primitive's wiring into the live coach entry path — hermetic unit tier carries the guarantee meanwhile (#1079 E031/E032/C002/M003) |

**Total classified:** 120 SMOKE acceptances (116 as of 2026-05-28; +4 #1079)
