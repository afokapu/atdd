...
| acc:mediate-worker-decisions:C010-SMOKE-001-post-1062-scoping-honored-live | real (live cmux + claude worker via the production launch builders; no mocks; reuses the surface-worker-decisions spawn harness) | a freedom-layer safe command (`pytest --version`) auto-runs with NO pending feed item while a gated command surfaces a pending permissionRequest — Claude Code honors the comma-delimited scoped allow-list at runtime | spawn (production builders) → worker runs safe vs gated Bash → cmux wrapper hook → feed.list (item present iff gated) | #1076 (S0 gate; the post-#1062 runtime-honor complement to #1062 E031/E032 emit-side; harness real + runnable, skips cleanly when cmux absent; **live verification pending coach run** — empirical anchor sampled only pre-#1062 coaches) |
| acc:mediate-worker-decisions:M006-SMOKE-001-live-gate-asserts-daemon-attached-before-handled | real (live cmux + claude worker publishes a genuine gated decision; real evaluate_mediation gate over a real ManagerRegistry-backed attach probe; no mocks) | with a real ManagerRegistry record (attached) the gate records HANDLED; with none (unattached) it flags the worker UNMEDIATED, never silently HANDLED | spawn → published permissionRequest → evaluate_mediation(decision, ManagerRegistry attach probe) → MediationStatus | #1076 (the #1084/A1 false-success the gate must catch; attach signal is a real ManagerRegistry record — full daemon-process liveness covered by coach-runtime M004/M005; harness real, skips cleanly when cmux absent; **live verification pending coach run**) |
