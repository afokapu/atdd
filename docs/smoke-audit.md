...
| acc:mediate-worker-decisions:D003-SMOKE-001-live-two-workers-no-cross-decide | real (two live cmux-native claude workers in two workspaces) | each workspace-scoped CmuxFeedSource returns only its own worker's decision; the two scoped result sets share no request_id (no cross-decide) | worker A / worker B → cmux Feed → per-workspace scoped CmuxFeedSource (surface.list identity) | #993 (live headline; reproduces the two-daemon cross-decide and proves the workspace scope isolates each consumer, evidence-bound per #983) |
