...
| acc:spawn-agents:E030-SMOKE-001-real-worker-memory-lands-outside-operator-config-dir | real (live claude CLI launched via the deployed `_inject_agent_env` env + subprocess; opt-in `ATDD_RUN_SMOKE=1`) | operator `~/.claude/.../-atdd-main/memory` gains no new file and MEMORY.md unchanged; worker memory lands in the isolated `CLAUDE_CONFIG_DIR` under the worktree runtime dir | spawn launch env (CLAUDE_CONFIG_DIR) → real claude worker → isolated config dir | #1057 (live verified — operator dir untouched, isolated dir populated; skips cleanly when claude CLI absent) |
...
| acc:mediate-worker-decisions:R004-SMOKE-001-real-run-replays-past-corrupt-tail | real (real on-disk §4.6 run dir + a fresh, independent `JsonlPersistenceStore`; no mocks) | a real persisted run whose `events.jsonl` has a truncated trailing line still replays, recovering every intact event in seq order | crash residue (truncated tail) → fresh JsonlPersistenceStore.replay_events → recovered events | #1084 B2 (real durable run files; runs live and passes — corrupt tail costs one record, not the run) |
| acc:mediate-worker-decisions:E015-SMOKE-001-real-fsync-survives-process-kill | real (real `append_jsonl` over real files in a child process killed with SIGKILL) | a record committed before the kill is present and whole on disk after the child dies | append_jsonl (temp + fsync + os.replace) → child SIGKILL → parent re-reads ledger | #1084 B1 (skipped: fsync durability is only falsifiable under an OS crash / power loss — a process SIGKILL keeps page-cache-flushed bytes; hermetic E015-UNIT-001/002 carry the all-or-nothing guarantee) |
| acc:mediate-worker-decisions:R005-SMOKE-001-live-daemon-survives-record-fault | real (live daemon process polling the Feed with its verdict-ledger path forced read-only) | the daemon stays alive and keeps polling after a verdict write fails, escalating rather than crashing | worker → cmux Feed → feed_daemon tick → record() fault → escalate-and-continue | #1084 C2 (skipped: needs a live spawned worker on the Feed (#967) + a long-running daemon, not inducible here; hermetic R005-UNIT-001/002 carry the survival guarantee) |
| acc:mediate-worker-decisions:K002-SMOKE-001-live-attach-failure-surfaces-blocked | real (live cmux dispatch whose worker daemon attach is induced to fail) | a durable BLOCKED decision + an escalation exist on disk for the unmediated worker; the dispatch is not reported HANDLED | dispatch → daemon attach fail → _escalate + _write_blocked_decision → decisions.jsonl/escalations.jsonl | #1084 A1 (skipped: needs a live cmux multiplexer + real spawned worker dispatch, not inducible here; hermetic K002-UNIT-001/002 carry the BLOCK-on-attach-failure guarantee) |
| acc:mediate-worker-decisions:K003-SMOKE-001-live-coach-escalation-persists | real (live coach run that raises a coach-side `_escalate`) | a durable escalation record for that reason survives on disk after the run, found after stderr scrolls away | coach run → _escalate → <runtime_root>/coach/escalations.jsonl | #1084 A0 (skipped: needs a live coach run loop to raise a real coach-side escalation, not inducible here; hermetic K003-UNIT-001/002 carry the durable-sink guarantee) |
