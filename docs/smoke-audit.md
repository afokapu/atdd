...
| acc:spawn-agents:E030-SMOKE-002-real-worker-reaches-task-not-onboarding-or-login | real (live claude CLI launched via the deployed `_inject_agent_env` env, now SEEDED, + subprocess; opt-in `ATDD_RUN_SMOKE=1`) | worker produces task output and does NOT park on the onboarding theme picker / 'Select login method' (auth + onboarding carried over via the seeded config dir) WHILE the operator memory dir still gains no new file | seeded isolated CLAUDE_CONFIG_DIR → real claude worker authenticates non-interactively → task output + clean operator dir | #1066 (strengthens SMOKE-001 which passed even when the worker died at login; live verified — worker reached task, operator dir untouched; skips cleanly when claude CLI absent) |
