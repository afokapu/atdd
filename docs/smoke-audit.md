...
| acc:mediate-worker-decisions:E011-SMOKE-001-live-decides-with-convention-loaded | real (live cmux + claude worker, real LlmCoach over claude -p; a thin recorder wraps the production claude provider CLI to capture the exact system context) | a verdict is produced AND the coach convention text was present in the decider's claude -p invocation | worker → cmux Feed → LlmCoach (claude -p --append-system-prompt coach-convention) → verdict | #987 (slice b live headline; decider decides AS A COACH, evidence-bound per #983; skips cleanly when cmux absent) |
