...
| acc:mediate-worker-decisions:M004-SMOKE-001-real-spawn-captures-daemon-output-to-log | real (a real detached child process spawned via the production SubprocessDaemonSpawner; /tmp scratch daemon.log; no cmux/claude needed) | the on-disk daemon.log contains the child's printed marker — its stdout was captured, not discarded to /dev/null | SubprocessDaemonSpawner.spawn(argv, log_path) → detached child stdout → daemon.log | #1007 (process-level observability smoke; proves a runtime failure leaves a diagnosable trace; runs anywhere) |
...
| acc:mediate-worker-decisions:M005-SMOKE-001-real-coach-start-managed-daemon-writes-a-verdict | real (live cmux + claude worker; the REAL `atdd coach start` detached SubprocessDaemonSpawner path — NOT the loop directly, NOT feed_daemon_cli foreground; a stale CMUX_SOCKET is injected only around the spawn to recreate the coach-session client-context the detached daemon must not inherit; /tmp scratch runtime root) | the MANAGED daemon's own verdicts.jsonl gains a line — the daemon spawned by the production command reached cmux cleanly and recorded a real decision | worker → `atdd coach start` (CoachRuntime.start) → SubprocessDaemonSpawner (detached, scrubbed env) → feed_daemon → CmuxFeedSource (cmux rpc) → LlmCoach (claude -p) → JsonlVerdictLedger (verdicts.jsonl) | #1007 reopened (the headline gate prior rounds missed: M003-SMOKE drove the loop directly and passed while the real command was broken; live verified 2026-06-08 — verdict written with the scrub, 0 verdicts + a loud CmuxCommandError trace in daemon.log without it; skips cleanly when cmux absent) |
