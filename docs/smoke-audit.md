...
| acc:author-atdd-substrate:K001-SMOKE-001-authored-artifact-validates | real (installed `atdd author` + `atdd validate` CLIs in a real checkout) | an artifact authored by the CLI is created in its canonical home and `atdd validate` passes over it unmodified (no authoring bypass) | atdd author write → atdd validate (normal suite) over authored artifact | #1097 (planned; authoring does not bypass validation) |
