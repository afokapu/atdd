...
def assert_fault_convention_only(
    ...
    """Inject the binding fault, prove the CONVENTION path alone catches it, revert.

    Used once a variant's legacy parity oracle is retired (#1207): the convention
    variant solely holds the coverage, so only the convention path is asserted (no
    legacy subprocess). The injection/evidence assertions are identical to
    `assert_fault_parity`; the legacy black-box run is simply dropped."""
    conv_path = root / conv_rel
    assert conv_path.exists(), f"convention not found: {conv_path}"
    ...
    assert conv_flags, f"{variant}: convention path did not catch the injected binding break"
