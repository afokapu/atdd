...
# =========================================================================== #
# Smoke-execution attestation (#1602)                                         #
# =========================================================================== #
#
# Everything above derives evidence from a commit. Everything below RECORDS a
# fact that a commit cannot express: that a live-smoke test executed.
#
# The distinction is the whole point of the issue. Before this, every artifact
# in the repo that called itself smoke evidence was either a static source scan
# (#1151's self-skip validator proves a test *cannot skip*, never that it *ran*)
# or an operator-typed stamp (``.atdd/smoke-evidence/<N>.yaml``, produced by
# ``atdd validate coder --smoke-required`` — a command that runs no test). Both
# are indistinguishable between "smoke ran against real infrastructure" and
# "somebody typed a command". An attestation written BY THE RUN, and only by the
# run, is the one artifact that tells them apart.
#
