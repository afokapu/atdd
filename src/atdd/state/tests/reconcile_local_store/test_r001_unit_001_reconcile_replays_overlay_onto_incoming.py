...
# Purpose: RED skeleton for acc:reconcile-local-store:R001-UNIT-001-reconcile-replays-overlay-onto-incoming — fails until the reconcile-local-store wagon implements it (train 0006). Refs #1400.
"""RED skeleton for acc:reconcile-local-store:R001-UNIT-001-reconcile-replays-overlay-onto-incoming.
...
def test_r001_unit_001_reconcile_replays_overlay_onto_incoming(tmp_path) -> None:
    """R001-UNIT-001-reconcile-replays-overlay-onto-incoming — behaviour not yet implemented."""
    raise AssertionError(
        "RED: reconcile-local-store acceptance acc:reconcile-local-store:R001-UNIT-001-reconcile-replays-overlay-onto-incoming is not implemented yet"
    )
