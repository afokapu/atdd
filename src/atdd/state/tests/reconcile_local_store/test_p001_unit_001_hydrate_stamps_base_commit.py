...
# Purpose: RED skeleton for acc:reconcile-local-store:P001-UNIT-001-hydrate-stamps-base-commit — fails until the reconcile-local-store wagon implements it (train 0006). Refs #1400.
"""RED skeleton for acc:reconcile-local-store:P001-UNIT-001-hydrate-stamps-base-commit.
...
def test_p001_unit_001_hydrate_stamps_base_commit(tmp_path) -> None:
    """P001-UNIT-001-hydrate-stamps-base-commit — behaviour not yet implemented."""
    raise AssertionError(
        "RED: reconcile-local-store acceptance acc:reconcile-local-store:P001-UNIT-001-hydrate-stamps-base-commit is not implemented yet"
    )
