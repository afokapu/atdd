...
# Purpose: RED skeleton for acc:reconcile-local-store:M001-SMOKE-001-head-change-reconcile — fails until the reconcile-local-store wagon implements it (train 0006). Refs #1400.
"""RED skeleton for acc:reconcile-local-store:M001-SMOKE-001-head-change-reconcile.
...
def test_m001_smoke_001_head_change_reconcile(tmp_path) -> None:
    """M001-SMOKE-001-head-change-reconcile — behaviour not yet implemented."""
    raise AssertionError(
        "RED: reconcile-local-store acceptance acc:reconcile-local-store:M001-SMOKE-001-head-change-reconcile is not implemented yet"
    )
