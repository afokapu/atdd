# Acceptance: acc:govern-lifecycle:E009-UNIT-004-the-ignore-entry-reaches-repos-initialised-before-it-existed
# Acceptance: acc:govern-lifecycle:E009-UNIT-005-the-ignore-entry-does-not-swallow-the-curated-ratchets
# Phase: RED
# Layer: integration
...
REQUIRED_ENTRIES = (
    ...
    # E009-UNIT-004: the per-run validation receipt. Its sibling
    # `.atdd/diagnostics/` has always been ignored; this path never was, so a
    # passing `atdd validate` dirtied the tree on every commit.
...
# =============================================================================
# CASE D — the entry must not swallow the curated ratchets (#1580 parity)
#
# `.atdd/baselines/` holds two opposite kinds of file. The per-phase validation
# receipt is derived — regenerated by every passing validate, read by nothing.
# `lint_toolkit.yaml`, `types_toolkit.yaml` and `four_tier_toolkit.yaml` are
# curated lists of frozen findings, and each one IS its gate's floor: absent,
# the gate passes on any amount of new debt.
#
# This is the shape of the 2026-07-20 mass-deletion, where an entry that looked
# correct took a tracked sibling down with it. So the guarantee is asserted
# against real git, not against the .gitignore text.
# =============================================================================

#: A derived per-run receipt — must be ignored.
RECEIPT_PATH = ".atdd/baselines/validation/planner.yaml"

#: Curated ratchets sharing the directory — must stay tracked.
RATCHET_PATHS = (
...
def test_seeding_ignores_the_receipt_but_not_the_curated_ratchets(tmp_path: Path) -> None:
    """E009-UNIT-005: the receipt is ignored; every ratchet stays trackable."""
    repo = _init_repo(tmp_path)
    ProjectInitializer(target_dir=repo)._seed_gitignore_entries()
    ...
    assert ignored(RECEIPT_PATH), (
        f"{RECEIPT_PATH} is not ignored — a passing `atdd validate` will keep "
        "dirtying the working tree after every commit"
    )

    for ratchet in RATCHET_PATHS:
        assert not ignored(ratchet), (
            f"{ratchet} is gitignored — it is a curated ratchet baseline, and a "
            "ratchet with no baseline passes on any amount of new debt (#1580)"
        )
