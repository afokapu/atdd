# URN: test:integration-hardening:coach-resume-wiring:E007-SMOKE-001-resume-real-orchestration
# Acceptance: acc:integration-hardening:E007-INTEGRATION-002-resume-spawns-pending-phase-persona
...
"""E007 SMOKE — verify the #734 resume-wiring fix against real infrastructure.

What this verifies that the E007 integration tests do not:

- The integration tests inject a *fake* ``transition_action`` through the
  ``_transition_action_override`` test seam. These smoke tests use the REAL
  ``_make_resume_transition_action`` that ``coach.py`` builds in production —
  so a regression that unwires the resume path (the exact #734 bug) is caught
  end-to-end, not just at the seam.
- ``_make_resume_transition_action`` dispatches to the REAL spawn handler,
  which loads the committed ``coder/refactor.prompt.yaml`` off disk. A missing
  or unparseable persona prompt would fail here — integration_002 stubs the
  prompt loader, so only a smoke test catches a broken prompt install.
- The kill-and-resume cycle is durable on the real filesystem: an original run
  logs INIT→…→SMOKE with the real ``DecisionWriter``, the writer is dropped to
  mimic process death, and a fresh ``coach.run(--resume)`` reconstructs from
  the on-disk ``decisions.jsonl`` and drives the pending phases for real.

The only boundary stubbed is the cmux pane backend (``_resolve_multiplexer`` →
``FakeMultiplexer``): a smoke test cannot open real terminal panes. Every other
layer — the action builder, the spawn handler, the prompt files, the decision
log — is real.
"""
...
def test_smoke_resume_drives_real_orchestration_on_real_fs(tmp_path, monkeypatch):
    ...
    worktree.mkdir()
    # Redirect the spawn handler's runtime + worktree resolution to real tmp
    # paths so it never touches the worktree's .atdd/runtime or hits GitHub.
    # The persona prompt loader is NOT redirected — the real committed
    # coder/refactor.prompt.yaml is loaded off disk.
...
def test_smoke_already_complete_resume_is_idempotent_on_real_fs(tmp_path, monkeypatch):
...
def test_smoke_blocked_resume_escalates_without_fast_forward_on_real_fs(
    ...
    """A resume whose pending phase genuinely cannot complete — the real spawn
    handler returns ERROR because the persona prompt is absent — BLOCKs: it
    writes a real escalation entry, returns non-zero, and stamps neither
    REFACTOR nor COMPLETE into the real decisions.jsonl."""
    from atdd.coach.commands.coach import run
    from atdd.coach.commands.durability import DecisionWriter
    from atdd.coach.handlers import spawn as spawn_handler
    ...
    # Genuine fault injection at the real boundary: point the persona prompt
    # loader at an empty (real) directory. The real spawn handler then raises
    # PromptNotFoundError → returns HandlerResult.ERROR → the real
    # _make_resume_transition_action raises → coach.py must BLOCK + escalate.
    empty_prompts = tmp_path / "no-prompts"
    empty_prompts.mkdir()
    monkeypatch.setattr(spawn_handler, "_PROMPTS_ROOT", empty_prompts)
    monkeypatch.setattr(spawn_handler, "_RUNTIME_ROOT", runtime_dir)
