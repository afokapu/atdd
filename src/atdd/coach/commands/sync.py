...
class AgentConfigSync:
    ...
    def sync(self, agents: Optional[List[str]] = None, force: bool = False) -> int:
        """
        Sync managed blocks to agent config files.

        Args:
            agents: List of agents to sync. If None, read from config.
            force: Allow removing lines that exist in the current managed block
                   but are absent from the new template output.

        Returns:
            0 on success, 1 on error or when net deletions are detected without --force.
        """
        ...
        for agent in agents:
            ...
            # Guard: detect net deletions from the managed block (#581).
            # When the new template would remove lines that the current block
            # contains (e.g. sections added by a prior PR), refuse to write
            # unless --force is explicitly passed.
            if self._has_managed_block(existing_content):
                removed_lines = self._detect_net_deletions(existing_content, new_block)
                if removed_lines and not force:
                    print(
                        f"Warning: {target_file}: sync would remove "
                        f"{len(removed_lines)} line(s) from the managed block "
                        f"that are not in the new template:"
                    )
                    for line in removed_lines[:10]:
                        print(f"  - {line}")
                    if len(removed_lines) > 10:
                        print(f"  ... and {len(removed_lines) - 10} more")
                    print(f"Run 'atdd sync --force' to allow removal.")
                    blocked_count += 1
                    continue
        ...
        if blocked_count:
            print(f"\nSync blocked: {blocked_count} file(s) have externally-added content.")
            print("Use 'atdd sync --force' to overwrite, or add missing content to the template/overlay.")
            return 1
    ...
    def _detect_net_deletions(self, existing_content: str, new_block: str) -> List[str]:
        """Return lines in the existing managed block that are absent from the new block.

        Only non-blank, non-marker lines are compared so whitespace-only churn
        never triggers a false positive.

        Args:
            existing_content: Current file content (with managed block).
            new_block: The newly rendered managed block.

        Returns:
            List of lines that would be removed (empty list if none).
        """
        existing_block, _, _ = self._extract_managed_block(existing_content)
        if existing_block is None:
            return []

        markers = {self.BLOCK_BEGIN, self.BLOCK_END}
        ...
        existing_lines = set(_significant_lines(existing_block))
        new_lines = set(_significant_lines(new_block))
