...
def _emit_gate_verdict(
    ...
    for hook in _gate_verdict_hooks:
        try:
            ...
            _logger.warning(
                "_emit_gate_verdict: Exception handled, continuing past the failure",
...
def _record_observed_violation(
    ...
    if not isinstance(namespace, dict):
        ...
        try:
            ...
            _logger.debug(
                "_record_observed_violation: (AttributeError, TypeError) handled, returning None",
                extra={"error": str(exc)[:200]},
            )
            return
...
def _record_rule_outcome(rule_id: str, outcome: str) -> None:
    ...
    if not isinstance(namespace, dict):
        ...
        try:
            ...
            _logger.debug(
                "_record_rule_outcome: (AttributeError, TypeError) handled, returning None",
                extra={"error": str(exc)[:200]},
            )
            return
...
def _read_line(repo_root: Path, rel_path: str, lineno: int) -> Optional[str]:
    ...
    try:
        ...
        _logger.warning(
            "_read_line: (OSError, UnicodeDecodeError) handled, returning None",
            extra={"error": str(exc)[:200]},
        )
        return None
    return None
...
def _emit_validator_reports(
    ...
    try:
        ...
        _logger.warning(
            "_emit_validator_reports: Exception handled, returning None",
            extra={"error": str(exc)[:200]},
        )
        return
