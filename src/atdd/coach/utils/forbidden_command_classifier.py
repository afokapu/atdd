...
def _load_registry(convention_path: Path) -> List[dict]:
    ...
    try:
        ...
        _logger.warning("forbidden_command_classifier: failed to load registry %s: %s", convention_path, exc)
        return []
...
def _check_and_record_loop_call(
    ...
    if state_file.exists():
        try:
            ...
            _logger.warning("forbidden_command_classifier: corrupt loop state, resetting: %s", exc)
            state = {}
    ...
    try:
        ...
        _logger.warning("forbidden_command_classifier: failed to write loop state: %s", exc)
...
def _write_audit(
    ...
    try:
        ...
        _logger.warning("forbidden_command_classifier: failed to write audit log: %s", exc)
