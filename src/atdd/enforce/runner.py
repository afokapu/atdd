...
def _substrate_provenance(repo_root: Path, substrate_home: Path) -> str:
    ...
    if Path(substrate_home) == _toolkit_root() and Path(substrate_home) != Path(repo_root):
        lines += [
...
def _no_bound_report(repo_root: Path, substrate_home: Path) -> str:
